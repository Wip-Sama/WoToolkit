...
class NotificationWindow : BorderPane() {
	...
	@FXML
	lateinit var basePane: BorderPane
	@FXML
	lateinit var dismiss: Button
	@FXML
	lateinit var icon: SVGPath
	@FXML
	lateinit var label: LocalizedLabel
	@FXML
	lateinit var progressBar: ProgressBar
	@FXML
	lateinit var progressBarContainer: BorderPane

	var notificationTypes: NotificationTypes by Delegates.observable(NotificationTypes.INFO) { _, oldValue, newValue ->
		if (oldValue != newValue) {
			...
			pseudoClassStateChanged(PseudoClass.getPseudoClass("success"), newValue == NotificationTypes.SUCCESS)
			pseudoClassStateChanged(PseudoClass.getPseudoClass("error"), newValue == NotificationTypes.ERROR)
			pseudoClassStateChanged(PseudoClass.getPseudoClass("warning"), newValue == NotificationTypes.WARNING)
			pseudoClassStateChanged(PseudoClass.getPseudoClass("info"), newValue == NotificationTypes.INFO)
		}
	}
	...
	init {
		...
		animateProgressBar(5000.0)
	}

	fun animateProgressBar(duration: Double) {

		Timeline(
			KeyFrame(Duration.ZERO, KeyValue(progressBar.progressProperty(), 0)),
			KeyFrame(Duration.millis(duration), KeyValue(progressBar.progressProperty(), 1))
		).play()
	}
}